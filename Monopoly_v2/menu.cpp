...
#define RTC_FLAG_ADDR 0  // адрес флага "RTC уже инициализирован"

// ---------------------------------------------------------
// ГЛОБАЛЬНЫЕ ПЕРЕМЕННЫЕ
// ---------------------------------------------------------
MenuState menuState = STATE_SPLASH;
...
HelpPage helpPages[] = {
  ...
  { "УПРАВЛЕНИЕ:", "Стрелки - листание", "Enter - подтверждение", "Esc - назад", "" },
...
DateTime safeNow() {
  ...
  return DateTime(2000, 1, 1, 0, 0, 0);  // нули, если RTC нет
}
...
void menuInit() {
  ...
  // Пытаемся запустить RTC
  if (rtc.begin()) {
    // Проверяем, идут ли секунды
    uint8_t s1 = readSecondsRaw();
    delay(1200);
    uint8_t s2 = readSecondsRaw();

    if (s1 != 0xFF && s2 != 0xFF && s1 != s2) {
      rtc_ok = true;  // RTC живой
    }
  }

  // Если RTC живой — устанавливаем время один раз
  if (rtc_ok) {
    byte flag = EEPROM.read(RTC_FLAG_ADDR);
    if (flag != 1) {
      rtc.adjust(DateTime(F(__DATE__), F(__TIME__)));
      EEPROM.write(RTC_FLAG_ADDR, 1);
      EEPROM.commit();
    }
  }

  // Если RTC мёртв — просто продолжаем работу
  // Время будет 00:00, таймер игры работает независимо

  menuState = STATE_SPLASH;
  uiShowSplash();
  delay(500);
}
...
void menuUpdate() {
  ...
  switch (menuState) {
    ...
    case STATE_SPLASH:
      static uint32_t splashStart = millis();
...
void setMenuState(MenuState newState) {
  ...
  switch (newState) {
    ...
    case STATE_SETTINGS_SYSTEM:
      drawScreen("СИСТЕМНЫЕ НАСТРОЙКИ", "Пока пусто", "", "", "[Esc] Назад");
      break;

    case STATE_SETTINGS_WIFI:
      drawScreen("WiFi", "Пока заглушка", "", "", "[Esc] Назад");
      break;
    ...
    case STATE_GAME_UNDO:
      if (!lastAction.valid) {
        uiShowGame_Undo_NoAction();
      } else {
        char l2[32], l3[32];
        snprintf(l2, sizeof(l2), "Игрок %d → %d",
                 lastAction.fromPlayer + 1,
                 lastAction.toPlayer + 1);
        snprintf(l3, sizeof(l3), "Сумма: %ld", lastAction.amount);
        uiShowGame_Undo_Action(l2, l3);
      }
      break;
...
// ---------------------------------------------------------
// long clampAmount(long value) {
//   if (value < 0) value = 0;
//   if (value > settings.maxBalance) value = settings.maxBalance;
//   if (value > 9999999) value = 666;  // защита от переполнения
//   return value;
// }
